...
from notifier.service import UserNotifierSettingsService
...
async def edit_mins_before_dt_start(message: Message, state: FSMContext):
    ...
    if not input_minutes:
        await message.answer(
            text="Invalid input, try again", reply_markup=edit_notify_settings_kb(False)
        )
        return
    curr_settings: UpdatedSettings = await state.get_value("user_settings")
...
async def save_updated_settings(callback: CallbackQuery, state: FSMContext):
    ...
    current_user_id = await state.get_value("user_id")
    await UserNotifierSettingsService.update_user_settings(current_user_id, current_settings)
...
async def get_user_settings(callback: CallbackQuery, state: FSMContext):
    ...
    if curr_state == NotifySettingsStates.edit:
        ...
        db_settings = await UserNotifierSettingsService.get_user_settings(user.id)
        user_settings = UpdatedSettings(**db_settings.to_dict())
        await state.set_data({"user_id": user.id, "user_settings": user_settings})
        is_edit = False
